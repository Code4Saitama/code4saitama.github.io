...
Facebookページのエクスポートデータ、公開FBグループの記録、写真素材、Deep Researchの補足調査をもとに、Code for SAITAMAの活動を時系列とテーマで統合したアーカイブをAstroで構築しています。
...
- `FB Page`: Facebookページのエクスポートに含まれるイベント・投稿
- `FB Group`: 公開Facebookグループから保存した投稿・写真
- `Deep Research`: 公開情報を追加調査して整理した補足情報

FB Group記録は目視レビュー済みの対象だけを保持し、既存イベントと重なる情報はイベント詳細へ、埼玉での活動は年表とテーマ別へ、外部事例や周辺情報は関連資料へ統合しています。活動単位の個別ページでは、匿名化済み本文・コメント・画像と元の公開投稿へのリンクを確認できます。
...
- 情報源タグを残し、FB Page、FB Group、Deep Research由来の記録を区別できるようにします。
...
Facebookグループからの取得は、ログイン済みブラウザを使う取得工程と、リポジトリ内で行うレビュー・匿名化・統合工程に分かれます。生のチェックポイントには投稿者名、プロフィールURL、期限付き画像URLが含まれるため、リポジトリへ追加しません。

再取得または別グループで同様のアーカイブを作る場合は、先に [Facebookグループの取得・レビュー・アーカイブ統合手順](docs/facebook-group-archive-workflow.md) を確認してください。ブラウザ取得、途中再開、コメント・イベント・写真の保存、非関連投稿の目視レビュー、個人情報の削除、既存記録との統合、公開前チェックまでをまとめています。

安全な処理順は次のとおりです。

1. 生データをリポジトリ外のチェックポイントへ取得する。
2. 本文・コメント・イベント・画像の取得漏れを確認する。
3. 非関連投稿を目視レビューしてからフィルターを適用する。
4. `npm run curate:group` で匿名化・要約・分類する。
5. `npm run export:content` と `npm run build` で統合結果を検証する。

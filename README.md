...
Facebookページのエクスポートデータ、公開FBグループの投稿、写真素材、DeepResearchの補足調査をもとに、イベントと投稿を統合したアーカイブをAstroで構築しています。

## 概要

- サイト種別: Astro による静的サイト
- 主な内容: イベント一覧、FBグループ投稿、年表、テーマ別整理、イベント詳細、団体紹介、補足資料
- コンテンツ形式: `src/content/` 以下の Markdown
- 画像素材: `public/assets/images/` 以下の original / webp
- 元データ: `data/facebook-export/` 以下のFacebookページデータ、`fb_group_archive/` 以下のFBグループデータ
...
FBグループ投稿は、目視レビューで削除対象に確定した38件を公開対象から除外しています。既存イベントとの統合は、正規化したイベント名が投稿本文に一致し、投稿日が開催日前後60日以内の場合に限定しています。
